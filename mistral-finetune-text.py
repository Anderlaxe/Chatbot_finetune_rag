...
train_dataset = load_database(path_database=PATH_TRAIN_SET_TEXT)
...
tokenized_train_dataset = train_dataset.map(generate_and_tokenize_prompt_text)
...
trainer = transformers.Trainer(
    ...
    args=transformers.TrainingArguments(
        output_dir=PATH_SAVE_MODEL_TEXT,
        warmup_steps=5,
        ...
        report_to="none",
        run_name=f"{MODE_TEXT}-{datetime.now().strftime('%Y-%m-%d-%H-%M')}"          # Name of the W&B run (optional)
    ),
