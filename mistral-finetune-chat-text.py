...
train_dataset_chat = load_database(path_database=PATH_TRAIN_SET_CHAT)
train_dataset_text = load_database(path_database=PATH_TRAIN_SET_TEXT)
train_dataset = merge_database(train_dataset_chat, train_dataset_text)
...
trainer = transformers.Trainer(
    ...
    args=transformers.TrainingArguments(
        output_dir=PATH_SAVE_MODEL_CHAT_TEXT,
        warmup_steps=5,
        ...
        report_to="none",
        run_name=f"{MODE_CHAT_TEXT}-{datetime.now().strftime('%Y-%m-%d-%H-%M')}"          # Name of the W&B run (optional)
    ),
