...
def inference(model, doc, question):
    text = f"Tu es le chatbot de l'université. Voici les documents à ta disposition : \n {doc}. Tes connaissances se limitent à ces documents. En te basant uniquement sur ces informations, réponds à la question suivante de manière précise et concise. Si la question ne concerne pas l'université (aussi appelé CS), le cursus ou la scolarité, ne répond pas à la question mais affiche seulement : 'Je suis désolé je ne comprends pas votre question'. Voici la question : \n {question}"
    ...
    try:
        ...
        while output[0] == '\n':
            output = output[1:] # Pour n'obtenir que la réponse
        return output
    except:
...
for info in tqdm.tqdm(dataset):
    ...
    answer = inference(model = model, doc = doc, question = question[:-1] + '?')
    print("answer : " + answer)
    ...
    with open('benchmark_ai_chatbot_cs/azure/results_mixtral.csv', 'a') as results:
